...
namespace NyAppWP.Pages.Camera
{
    ...
    public abstract class RangeParameter<T> : CameraParameter
    {
    ...
    public class ExposureCompensationParameter : RangeParameter<Int32>
    {
        ...
        {
            if (CameraDataContext.Settings.Contains(ParameterSettingName))
            {
                if ((Int32)CameraDataContext.Settings[ParameterSettingName] != Value)
                {
                    CameraDataContext.Settings[ParameterSettingName] = Value;
                }
            }
            else
            {
                CameraDataContext.Settings.Add(ParameterSettingName, Value);
            }
        }
        ...
        public override bool Load()
        {
            ...
            if (CameraDataContext.Settings.Contains(ParameterSettingName))
            {
                Int32 value = (Int32)CameraDataContext.Settings[ParameterSettingName];
                this.Value = value;
                ret = true;
            }
            return ret;
    ...
    public class FlashPowerParameter : RangeParameter<UInt32>
    {
        ...
        public override void Save()
        {
            if (CameraDataContext.Settings.Contains(ParameterSettingName))
            {
                if ((UInt32)CameraDataContext.Settings[ParameterSettingName] != Value)
                {
                    CameraDataContext.Settings[ParameterSettingName] = Value;
                }
            }
        ...
        public override bool Load()
        {
            ...
            if (CameraDataContext.Settings.Contains(ParameterSettingName))
            {
                UInt32 value = (UInt32)CameraDataContext.Settings[ParameterSettingName];
                this.Value = value;
                ret = true;
            }
            return ret;
